<?php
  ...
  if (isset($_POST["fname"]) && isset($_POST["lname"]) && isset($_POST["email"]) && isset($_POST["pass"])) {
    ...
    if (!preg_match("/^[a-zA-Z-' ]*$/",$_POST["fname"])) {
        ...
        $addError = "you must insert an address";
    }elseif (!isset($_POST["pnum"])) {
        $phoneError = "please insert a phone number";
    }else {
        addUser($_POST["fname"] ,$_POST["lname"] , 0 ,$_POST["pnum"] ,$_POST["address"] ,$_POST["email"] ,$_POST["pass"]);
    }
    ...
    <form class="container mt-2 is-validated" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
      ...
      <div class="form-group">
        ...
        <?php if(isset($addError)){?>
          <label class="alert alert-danger"><?php echo $addError; ?></label>
        <?php } ?>
      </div>
      <div class="form-group">
        <label>Phone Number:</label>
        <input type="text" name="pnum" class="form-control" value="<?php if(isset($_POST["pnum"])) {echo $_POST["pnum"];} ?>">
        <?php if(isset($phoneError)){?>
          <label class="alert alert-danger"><?php echo $phoneError; ?></label>
        <?php } ?>
      </div>
      <div class="form-group">
